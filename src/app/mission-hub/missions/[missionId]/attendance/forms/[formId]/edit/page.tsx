...
export default function EditAttendanceFormPage() {
    ...
    const loadForm = async () => {
      try {
        ...
        if (result.success) {
          ...
          setNotification({ type: 'error', message: 'Failed to load form' });
        }
      } catch (error) {
  ...
  const updateForm = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setNotification({ type: 'error', message: result.error || 'Failed to update form' });
      }
    } catch (error) {
  ...
  return (
    <div className="p-6" onClick={closeAllTooltips}>
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Edit Attendance Form</h1>
          <p className="text-gray-600">Modify the attendance form for this mission.</p>
        </div>
      </div>
